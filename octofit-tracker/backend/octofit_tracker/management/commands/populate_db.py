...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # Create activities
        activities = [
            {"_id": ObjectId(), "user_id": users[0]["_id"], "activity_type": 'Cycling', "duration": timedelta(hours=1).total_seconds()},
            {"_id": ObjectId(), "user_id": users[1]["_id"], "activity_type": 'Crossfit', "duration": timedelta(hours=2).total_seconds()},
            {"_id": ObjectId(), "user_id": users[2]["_id"], "activity_type": 'Running', "duration": timedelta(hours=1, minutes=30).total_seconds()},
            {"_id": ObjectId(), "user_id": users[3]["_id"], "activity_type": 'Strength', "duration": timedelta(minutes=30).total_seconds()},
            {"_id": ObjectId(), "user_id": users[4]["_id"], "activity_type": 'Swimming', "duration": timedelta(hours=1, minutes=15).total_seconds()},
        ]
